...
public:
  ...
  void
  set_handler(WBSViewCallbacks* handler)
  {
    m_handler = handler;
  }

  void
  enable_add_child(bool enable);
  void
  enable_add_sibling(bool enable);
  void
  enable_indent_task(bool enable);
  void
  enable_unindent_task(bool enable);
  void
  enable_up(bool enable);
  void
  enable_down(bool enable);
  void
  enable_cut(bool enable);
  void
  enable_copy(bool enable);
  void
  enable_paste(bool enable);
  void
  enable_delete(bool enable);

  void
  add_child(const WBS::Path& path);
  void
  add_sibling(const WBS::Path& t_path);
  void
  indent(const WBS::Path& path);
  void
  unindent(const WBS::Path& path);
  void
  renumber(void);
...
private:
  ...
  void
  cb_on_button_pressed(GdkEventButton* event);
  void
  cb_on_row_activated(
    const Gtk::TreeModel::Path& path,
    Gtk::TreeViewColumn* column);
  void
  cb_on_add_child_clicked(void);
  void
  cb_on_add_sibling_clicked(void);
  void
  cb_on_cut_clicked(void);
  void
  cb_on_copy_clicked(void);
  void
  cb_on_paste_clicked(void);
  void
  cb_on_delete_clicked(void);
  void
  cb_on_indent_clicked(void);
  void
  cb_on_unindent_clicked(void);
  void
  cb_on_up_clicked(void);
  void
  cb_on_down_clicked(void);
  void
  cb_on_row_changed(
    const Gtk::TreeModel::Path& path,
